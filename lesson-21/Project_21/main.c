...
//    GPIOF_AHB_BITS[LED_BLUE] = LED_BLUE;
    ...
    SysTick_Handler(); // Appreciate the ability to call any interrupt in Cortex-M. On other architectures, need to manually set timer increment to 1 tick before trigger
                       // With FPU enabled, need larger stack (larger stack allocation size) (longer entry/exit time)
    
    __enable_irq();
    while (flashes <= 100) {
      GPIOF_AHB_BITS[LED_GREEN] = LED_GREEN; //By bit-masking, we make the LED write atomic, preventing a race-condition from shared resource
